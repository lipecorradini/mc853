...
Este relatório descreve o banco de dados utilizado para desenvolver um modelo preditivo de sobrevivência para pacientes com esquistossomose, baseado nas bases de dados fornecidas (esqubr21.csv, esqubr22.csv, esqubr23.csv, esqubr20.csv). O objetivo do modelo é prever, a partir de variáveis demográficas, clínicas e epidemiológicas, a probabilidade de sobrevivência dos pacientes.

## 1. Descrição do Banco de Dados

As bases de dados contêm registros de casos notificados de esquistossomose no Brasil entre 2020 e 2023. Cada registro inclui informações sobre a notificação, características demográficas, resultados de exames e tratamentos administrados.
...
1. **Tratamento de valores ausentes**:
   - Para variáveis como FORM, EVOLUCAO e AN_QUANT, foram aplicadas estratégias de imputação, utilizando a mediana e a moda onde aplicável.
   - Remoção de casos com mais de 30% de valores ausentes.

2. **Remoção de outliers**:
   - Valores extremos em AN_QUANT (contagem de ovos) foram identificados e tratados utilizando o método do intervalo interquartil (IQR).
...
3. **Normalização de variáveis numéricas**:
   - Log-transformação aplicada a AN_QUANT devido à sua distribuição assimétrica.
...
- **Distribuição**: Predominância de casos em pacientes nascidos entre 1950-2000, refletindo maior exposição em adultos em idade produtiva.
...
### 3.3. CS_GESTANT
- **Tipo**: Categórico binário
- **Distribuição**: Aproximadamente 10% dos casos femininos são em gestantes.
...
### 3.5. AN_QUANT
- **Tipo**: Numérico (inteiro)
- **Distribuição**: Assintótica à direita (distribuição log-normal), com maioria dos casos apresentando contagens baixas a moderadas.
...
### 3.7. TRATAM
- **Tipo**: Categórico nominal
- **Distribuição**: Predominância de praziquantel (≈85%).
...
### 3.9. FORMA
- **Tipo**: Categórico nominal
- **Distribuição**: Predominância da forma intestinal (≈70%).
- **Relevância**: Principal determinante da morbimortalidade.

### 3.10. COUFINF
- **Tipo**: Categórico nominal
- **Distribuição**: Concentração em estados endêmicos do Nordeste e Sudeste.
...
- **TRATAM e sobrevivência**: Correlações fracas a moderadas.

### 4.4. Interações Relevantes
- **Idade x FORMA**: Interação significativa entre formas graves em idades avançadas e o prognóstico.
